...
async function hacerEnvio(email, password) {
  ...
  try {
    ...
    await context.overridePermissions("https://pymes.andreani.com", [
      "geolocation",
    ]);
    ...
    page.on("request", (request) => {
      ...
      if (
        url.includes("pymes-api.andreani.com") ||
    ...
    while (!navegacionExitosa && intentosNavegacion < maxIntentosNavegacion) {
      try {
        intentosNavegacion++;
        console.log(
          `🔍 Intento de navegación ${intentosNavegacion} de ${maxIntentosNavegacion}...`
        );
        ...
        console.log(
          "📸 Capturando screenshot después de navegación exitosa..."
        );
        ...
        console.log(
          `📸 Capturando screenshot de error de navegación intento ${intentosNavegacion}...`
        );
        try {
          await page.screenshot({
          ...
          console.log(
            "⚠️ No se pudo tomar screenshot del error:",
    ...
    console.log(
      "📸 Capturando screenshot antes de buscar selector #signInName..."
    );
    await page.screenshot({
    ...
    console.log(
      ...
      JSON.stringify(analisisDOM, null, 2)
    );
    ...
    if (analisisDOM.signInNameEncontrado.existe) {
      console.log(
        "✅ Elemento #signInName encontrado, procediendo con login normal..."
      );
      try {
        await page.waitForSelector("#signInName", {
          visible: true,
    ...
    if (!loginExitoso) {
      console.log(
        "🔄 Intentando estrategias alternativas para encontrar campos de login..."
      );

      // 📸 CAPTURA 3: Estado cuando no se encuentra signInName
      console.log(
        "📸 Capturando screenshot cuando no se encuentra #signInName..."
      );
      await page.screenshot({
      ...
      for (const estrategia of estrategias) {
        try {
          ...
          if (elementoEmail && elementoPassword) {
            const sonVisibles = await page.evaluate(
              (selEmail, selPassword) => {
                ...
                return (
                  rectEmail.width > 0 &&
                  rectEmail.height > 0 &&
                  rectPassword.width > 0 &&
                  rectPassword.height > 0
                );
              },
              estrategia.email,
              estrategia.password
            ...
            if (sonVisibles) {
              ...
              console.log(
                "📸 Capturando screenshot con elementos alternativos..."
              );
              await page.screenshot({
              ...
              await page.waitForSelector(estrategia.email, {
                visible: true,
                timeout: 10000,
              });
              loginExitoso = true;
              console.log(
                `🔄 Usando selectores: email=${selectorEmailUsado}, password=${selectorPasswordUsado}`
              );
              break;
            }
          }
    ...
    if (!loginExitoso) {
      ...
      throw new Error(
        "No se pudo encontrar ningún campo de login en la página"
      );
    }

    // Proceder con el login
    console.log(
      `✅ Usando selectores: email=${selectorEmailUsado}, password=${selectorPasswordUsado}, boton=${selectorBotonUsado}`
    );
    ...
    console.log(
      "📸 Capturando screenshot antes de buscar botón 'Hacer envío'..."
    );
    await page.screenshot({
    ...
    console.log(
      "📸 Capturando screenshot después del click en 'Hacer envío'..."
    );
    await page.screenshot({
    ...
    if (successfulRequests.length > 0) {
      console.log(
        "🟢 REQUESTS EXITOSAS:",
        JSON.stringify(successfulRequests.slice(-3), null, 2)
      );
    }
    if (failedRequests.length > 0) {
      console.log(
        "🔴 REQUESTS FALLIDAS:",
        JSON.stringify(failedRequests.slice(-3), null, 2)
      );
    }
    ...
    try {
      await page.waitForSelector(
        '.leaflet-container, [class*="map"], [class*="Map"]',
        {
          visible: true,
          timeout: 30000,
        }
    ...
    const sucursalesIniciales = await page.evaluate(() => {
      ...
      return {
        ...
        haySeleccionada: !!document.querySelector(
          '[data-testid="branch-card"][data-selected="true"]'
        ),
      };
    });
    ...
    console.log(
      "🏪 Sucursales iniciales detectadas:",
      JSON.stringify(sucursalesIniciales, null, 2)
    );
    ...
    if (
      sucursalesIniciales.cantidad > 0 &&
      sucursalesIniciales.haySeleccionada
    ) {
      console.log(
        "🎯 Ya hay una sucursal preseleccionada automáticamente, saltando escritura de dirección"
      );
      ...
      console.log(
        "✅ Campo de dirección encontrado, analizando inputs disponibles..."
      );
      ...
      if (inputsInfo.length > 0) {
        ...
        const primerInput = inputsInfo.find(
          (input) => input.visible && !input.disabled
        );
        if (primerInput) {
          console.log(
            `📍 Intentando escribir en input ${primerInput.index}...`
          );
          ...
          try {
            ...
            await page.keyboard.type(
              "Azcuénaga 1001, C1115AAE Ciudad de Buenos Aires, Argentina",
              { delay: 100 }
            );
            ...
            console.log(
              `⚠️ Error escribiendo en input ${primerInput.index}:`,
              error.message
            );
          }
        }
      ...
      if (!direccionEscrita) {
        console.log(
          "⚠️ No se pudo escribir con evaluate, intentando método click + type..."
        );
        try {
          await page.click(".MuiInputBase-input");
          await page.keyboard.selectAll();
      ...
      if (direccionEscrita) {
        console.log(
          "⏳ Esperando a que aparezcan las opciones de dirección..."
        );

        // Esperar a que aparezcan las opciones en el dropdown/lista
        try {
          await page.waitForSelector(
            '.MuiAutocomplete-option, .MuiMenuItem-root, li[role="option"], [role="option"]',
            {
              visible: true,
              timeout: 10000,
            }
          ...
          console.log(
            "🏠 Opciones de dirección encontradas:",
            JSON.stringify(opciones.slice(0, 5), null, 2)
          );
          ...
          console.log(
            "⚠️ No aparecieron opciones de dirección:",
            error.message
          );
      ...
      console.log(
        ...
        JSON.stringify(verificacionDireccion, null, 2)
      );
      ...
      if (successfulRequests.length > 0) {
        console.log(
          "🟢 ÚLTIMAS REQUESTS EXITOSAS:",
          JSON.stringify(successfulRequests.slice(-5), null, 2)
        );
      }
      ...
      try {
        ...
        console.log(
          "⚠️ Timeout esperando sucursales, analizando requests de red..."
        );
        ...
        if (successfulRequests.length > 0) {
          console.log(
            "✅ REQUESTS EXITOSAS:",
            JSON.stringify(successfulRequests.slice(0, 5), null, 2)
          );
        }

        // Intentar usar ubicación actual como alternativa
        console.log(
          "🔄 Intentando usar 'Utilizar ubicación actual' como alternativa..."
        );
        try {
          // Buscar botón por clase primero (más confiable)
          const ubicacionBtnClass = await page.$(
            ".SearchLocation_myLocation__o9yDP"
          );
          ...
          console.log(
            "⚠️ No se pudo hacer click en ubicación actual:",
            error.message
          );
        }
    ...
    const elementosEncontrados = await page.evaluate(() => {
      ...
      const branchCards = document.querySelectorAll(
        '[data-testid="branch-card"]'
      );
    ...
    console.log(
      "📋 Branch cards encontradas:",
      JSON.stringify(elementosEncontrados.branchCards, null, 2)
    );
    console.log(
      "🏢 Elementos con 'branch':",
      JSON.stringify(
        elementosEncontrados.elementosConBranch.slice(0, 10),
        null,
        2
      )
    );
    console.log(
      "🏪 Elementos con 'sucursal':",
      JSON.stringify(
        elementosEncontrados.elementosConSucursal.slice(0, 10),
        null,
        2
      )
    );
    console.log(
      "🏷️ Todos los data-testid:",
      JSON.stringify(elementosEncontrados.todosLosTestIds.slice(0, 20), null, 2)
    );
    console.log(
      "👆 Elementos clickeables:",
      JSON.stringify(
        elementosEncontrados.elementosClickeables.slice(0, 10),
        null,
        2
      )
    );
    ...
    while (!sucursalEncontrada && intentos < maxIntentos) {
      try {
        intentos++;
        console.log(
          `🔍 Intento ${intentos} de ${maxIntentos} para encontrar sucursal preseleccionada...`
        );
        ...
        await page.waitForFunction(
          () => {
            ...
            for (let div of allDivs) {
              if (
                div.innerText &&
                (div.innerText.includes("sucursal") ||
                  div.innerText.includes("Sucursal"))
              ) {
        ...
        console.log(
          `📸 Capturando screenshot del intento ${intentos} fallido...`
        );
        await page.screenshot({
        ...
        console.log(
          `🔍 Intento ${intentos} falló, analizando DOM nuevamente...`
        );
        ...
        const analisisDetallado = await page.evaluate(() => {
          ...
          Array.from(posiblesAlternativas)
            .slice(0, 10)
            .forEach((el, index) => {
        ...
        console.log(
          `🔍 ANÁLISIS DETALLADO INTENTO ${intentos}:`,
          JSON.stringify(analisisDetallado, null, 2)
        );

        // Intentar seleccionar sucursal automáticamente con múltiples métodos
        const sucursalSeleccionada = await page.evaluate(() => {
          console.log(
            "🔍 Iniciando búsqueda de sucursales para selección automática..."
          );
          ...
          const branchContainer = document.querySelector(
            ".Branches_container__WBihE"
          );
          if (branchContainer) {
            const firstChild = branchContainer.querySelector("div:first-child");
            if (firstChild) {
              console.log(
                "📍 Primer elemento en contenedor de sucursales encontrado"
              );
              firstChild.click();
          ...
          console.log(
            `🔍 Verificando ${cards.length} tarjetas con data-testid="branch-card"...`
          );
          ...
          if (cards.length > 0) {
            ...
            if (
              !primeraCard.getAttribute("data-selected") ||
              primeraCard.getAttribute("data-selected") !== "true"
            ) {
              console.log(
                "👆 Haciendo click en primera tarjeta data-testid..."
              );
              primeraCard.click();
          ...
          for (let element of allElements) {
            if (
              element.innerText &&
              element.innerText.toLowerCase().includes("sucursal")
            ) {
              const rect = element.getBoundingClientRect();
              if (rect.width > 0 && rect.height > 0) {
                console.log(
                  '📍 Elemento con texto "sucursal" encontrado:',
        ...
        if (sucursalSeleccionada && sucursalSeleccionada.success) {
          ...
          throw new Error(
            `No se pudo encontrar o seleccionar una sucursal después de ${maxIntentos} intentos`
          );
        } else {
          console.log("⏳ Esperando antes del siguiente intento...");
    ...
    const waitEnvioResponse = page.waitForResponse(
      (res) => {
        try {
          return (
            res
              .url()
              .startsWith("https://pymes-api.andreani.com/api/v1/Envios") &&
    ...
    // Hacemos click y esperamos la respuesta del POST en paralelo
    await page.waitForSelector("#finalizar_envio--paquetes", { visible: true });
    console.log("🎯 Haciendo click en 'Finalizar envío'...");
    const [envioRes] = await Promise.all([
      waitEnvioResponse,
      page.click("#finalizar_envio--paquetes"),
    ]);

    // Parseamos el body del POST (Puppeteer ya descomprime si viene gzip)
    try {
      const ct = (envioRes.headers()["content-type"] || "").toLowerCase();
      if (ct.includes("application/json")) {
        responseData = await envioRes.json();
      } else {
        const txt = await envioRes.text();
        responseData = txt ? JSON.parse(txt) : null;
      }
      console.log("✅ POST /Envios body:", responseData);
    } catch (e) {
      console.log("⚠️ No pude parsear el body del POST /Envios:", e.message);
    }

    // Fallback por si (raro) viniera sin body pero con Location
    if (!responseData) {
      const locationHeader =
        envioRes.headers()["location"] ||
        envioRes.headers()["content-location"];
      if (locationHeader && authToken) {
        const apiBase = "https://pymes-api.andreani.com";
        const absoluteUrl = locationHeader.startsWith("http")
          ? locationHeader
          : `${apiBase}${locationHeader}`;
        try {
          console.log("🔎 GET al recurso creado vía Location:", absoluteUrl);
          const r = await fetch(absoluteUrl, {
            headers: { Authorization: authToken, Accept: "application/json" },
          });
          if (r.ok) {
            responseData = await r.json();
            console.log("✅ Datos obtenidos por GET Location");
          } else {
            console.log("ℹ️ GET Location no OK:", r.status);
          }
        } catch (e) {
          console.log("ℹ️ Error GET Location:", e.message);
        }
      }
    }
    ...
    return {
      ...
      pedidoId: responseData?.pedidoId || urlPedidoId || null,
...
DOM ANALYSIS: ${JSON.stringify(errorAnalysis, null, 2)}
        ...
        fs.writeFileSync(
          `/app/screenshots/error-log-${timestamp}.txt`,
          errorLog
        );
        console.log(`📝 Log de error guardado en: error-log-${timestamp}.txt`);
      } catch (screenshotError) {
        console.log(
          "⚠️ Error tomando screenshots de debugging:",
          screenshotError.message
        );
      }
    }
