...
async function hacerEnvio(email, password) {
  ...
  try {
    ...
    // En producción o Docker, usar executablePath personalizado
    ...
    // Escribir credenciales con delay simulado
    await page.fill("#signInName", ""); // Limpiar primero
    await page.type("#signInName", finalEmail, { delay: 60 });
    
    await page.fill("#password", ""); // Limpiar primero
...
app.listen(port, () => {
  ...
  console.log(
    `🌐 Chromium path: ${process.env.CHROMIUM_PATH || "No configurado (usando Playwright bundled)"}`
  );
});
